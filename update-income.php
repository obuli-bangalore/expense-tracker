...
$result = mysqli_query($con, "SELECT * FROM tblincome WHERE ID=$rowid");

if (count($result) > 0)
{
	$res = mysqli_fetch_array($result);

	$dates = $res['IncomeDate'];
	$items = $res['IncomeItem'];
	$costs = $res['IncomeCost'];
...
if (isset($_POST['update'])) {
	...
	$query=mysqli_query($con, "UPDATE tblincome SET  IncomeDate = '$date', IncomeItem='$item', IncomeCost = '$amount'  WHERE id=$rowid");
	if($query)
	{
